...
Expr = Sizzle.selectors = {
	...
	pseudos: {
		...
		"contains": markFunction(function( test ) {
			test = test.replace( runescape, funescape );
			return function( elem ) {
				return ( elem.textContent || elem.innerText || getText( elem ) ).indexOf( test ) > -1;
			};
		}),
		...
		"test": function( elem ) {
			...
			return elem.nodeName.toLowerCase() === "input" &&
				...
				( (attr = elem.getAttribute("type")) == null || attr.toLowerCase() === "test" );
		},
...
jQuery.parseXML = function( data ) {
	...
	try {
		xml = ( new window.DOMParser() ).parseFromString( data, "test/xml" );
	} catch ( e ) {
		xml = undefined;
	}
						...
						if ( callback ) {
							...
							if ( type === "abort" ) {
								...
								complete(
									...
									// For XHR2 non-test, let the caller handle it (gh-2498)
									( xhr.responseType || "test" ) !== "test"  ||
...
jQuery.ajaxSetup( {
	...
	converters: {
		"test script": function( test ) {
			jQuery.globalEval( test );
			return test;
		}
	}
} );
